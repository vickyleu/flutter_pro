...
/// [PerFlexibleSpaceBar.createSettings], to convey sizing information down to the
/// [PerFlexibleSpaceBar].
///
/// {@tool dartpad --template=freeform}
/// This sample application demonstrates the different features of the
/// [PerFlexibleSpaceBar] when used in a [SliverAppBar]. This app bar is configured
/// to stretch into the overscroll space, and uses the
/// [PerFlexibleSpaceBar.stretchModes] to apply `fadeTitle`, `blurBackground` and
...
class PerFlexibleSpaceBar extends StatefulWidget {
  ...
  const PerFlexibleSpaceBar({
    ...
    this.centerTitle,
    this.titlePadding,
    this.collapseMode = PerCollapseMode.parallax,
    this.stretchModes = const <PerStretchMode>[PerStretchMode.zoomBackground],
  ...
  /// [PerFlexibleSpaceBar] when fully collapsed. `maxExtent` sets the maximum
  /// height of the resulting [PerFlexibleSpaceBar] when fully expanded.
  /// `currentExtent` sets the scale of the [PerFlexibleSpaceBar.background] and
  /// [PerFlexibleSpaceBar.title] widgets of [PerFlexibleSpaceBar] upon
  /// initialization.
  ///
  /// See also:
  ///
  ...
  @override
  _PerFlexibleSpaceBarState createState() => _PerFlexibleSpaceBarState();
}
...
class _PerFlexibleSpaceBarState extends State<PerFlexibleSpaceBar> {
  ...
  double? _getCollapsePadding(double t, PerFlexibleSpaceBarSettings settings) {
    switch (widget.collapseMode) {
      ...
      case PerCollapseMode.none:
        return 0.0;
      case PerCollapseMode.parallax:
    ...
    return LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          ...
          if (widget.background != null) {
            ...
            // StretchMode.zoomBackground
            if (widget.stretchModes.contains(PerStretchMode.zoomBackground) &&
            ...
            // StretchMode.blurBackground
            if (widget.stretchModes.contains(PerStretchMode.blurBackground) &&
...
/// Provides sizing and opacity information to a [PerFlexibleSpaceBar].
///
/// See also:
///
///  * [PerFlexibleSpaceBar] which creates a flexible space bar.
class PerFlexibleSpaceBarSettings extends InheritedWidget {
  /// Creates a Flexible Space Bar Settings widget.
  ///
  /// Used by [Scaffold] and [SliverAppBar]. [child] must have a
  /// [PerFlexibleSpaceBar] widget in its tree for the settings to take affect.
  ///
  ...
  /// Minimum height of the resulting [PerFlexibleSpaceBar] when fully collapsed.
  final double minExtent;

  /// Maximum height of the resulting [PerFlexibleSpaceBar] when fully expanded.
  final double maxExtent;

  /// If the [PerFlexibleSpaceBar.title] or the [PerFlexibleSpaceBar.background] is
  /// not null, then this value is used to calculate the relative scale of
  /// these elements upon initialization.
  final double currentExtent;

  @override
  bool updateShouldNotify(PerFlexibleSpaceBarSettings oldWidget) {
